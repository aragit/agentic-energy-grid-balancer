...
class GridSimulation:
    def __init__(self, llm, db_session=None, steps: int = 24):
        ...
        self.generators = [
            ...
            if a.agent_type in ["solar", "wind", "coal", "nuclear"]
        ]
        self.battery = next((a for a in self.agents if a.agent_type == "battery"), None)
        self.consumer = next(
            (a for a in self.agents if a.agent_type == "consumer"), None
        )
    ...
    def run(self) -> SimulationResult:
        ...
        logger.info(
            f"[SIMULATION] Complete. Final frequency: {result.final_frequency} Hz"
        )
        return result
    ...
    def _run_step(self, step: int) -> tuple[float, float]:
        ...
        for gen in self.generators:
            output = gen.compute_output(
                weather,
                self.auction.price_history[-1] if self.auction.price_history else 50.0,
            )
        ...
        if self.battery:
            ...
            if output_adj == "charge" and self.battery.get_available_charge() > 0.01:
                ...
                logger.info(
                    f"[BATTERY BID] BUY {self.battery.get_available_charge():.1f} MW at {bid_price:.2f}"
                )
            elif (
                output_adj == "discharge"
                and self.battery.get_available_discharge() > 0.01
            ):
                bids.append(
                    Bid(
                        agent_name=self.battery.name,
                ...
                logger.info(
                    f"[BATTERY BID] SELL {self.battery.get_available_discharge():.1f} MW at {bid_price:.2f}"
                )
            else:
        ...
        for tx in clearing.transactions:
            ...
            if buyer:
                if buyer.agent_type == "battery":
                    ...
                    actual_demand_met += tx[
                        "quantity_mwh"
                    ]  # Battery charging is grid demand
        ...
        if not self.physics.is_stable(frequency):
            ...
            if action["action"] == "dispatch":
                frequency = self.physics.compute_frequency(
                    imbalance + action["dispatch_mw"]
                )
    ...
    def _compute_marginal_cost(self, gen: BaseAgent) -> float:
        ...
        carbon_cost = (
            gen.state.carbon_intensity_g_kwh / 1000
        ) * self.auction.carbon_price
