...
class RegulatoryAgent:
    ...
    def check_frequency(self, frequency: float, step: int) -> bool:
        if frequency < 49.0 or frequency > 51.0:
            violation = {"step": step, "type": "frequency", "value": frequency, "limit": "49.0-51.0 Hz", "severity": "CRITICAL"}
            self.violations.append(violation)
            logger.error(f"[REGULATOR] CRITICAL: Frequency {frequency:.3f} Hz out of bounds!")
            return False
        return True

    def check_carbon(self, agent, step: int) -> bool:
        if agent.state.total_carbon_emitted > self.carbon_cap:
            violation = {"step": step, "type": "carbon", "agent": agent.name, "emitted": agent.state.total_carbon_emitted, "cap": self.carbon_cap, "severity": "VIOLATION"}
            self.violations.append(violation)
    ...
    def check_market_fairness(self, bids: List[Dict], step: int) -> bool:
        ...
        if max(prices) - min(prices) < 0.01:
            violation = {"step": step, "type": "market_manipulation", "detail": "All bids identical", "severity": "SUSPICIOUS"}
            self.violations.append(violation)
    ...
    def get_summary(self) -> Dict:
        ...
        return {"total_violations": len(self.violations), "critical": len(critical), "violations": len(violations), "details": self.violations}
