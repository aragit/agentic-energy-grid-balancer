...
class ConsumerAgent(BaseAgent):
    def __init__(self, name: str, base_demand_mw: float, llm):
        super().__init__(name, "consumer", base_demand_mw, carbon_intensity_g_kwh=0.0, llm=llm)
    ...
    def compute_output(self, weather, market_price: float) -> float:
        demand = self.demand_model.compute(hour=0, temperature=weather.temperature, price=market_price)
